  ...
  return (
    <div className="mx-auto max-w-5xl p-5">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-white">{user.role === "citizen" ? "My Cases" : "Cases"}</h2>
        <div className="ml-auto flex items-center gap-1 rounded-lg bg-ink-900/60 p-1 text-xs">
          {["all", "open", "active", "closed"].map((s) => (
            <button key={s} onClick={() => setFilter(s)} className={`rounded px-2.5 py-1 ${filter === s ? "bg-accent-600 text-white" : "text-slate-400"}`}>{s}</button>
          ))}
        </div>
        {isLead(user) && (
      ...
      {loading ? (
        ...
        <p className="py-10 text-center text-sm text-slate-500">No cases.</p>
      ) : (
            ...
            <button key={c.id} onClick={() => setActive(c.id)} className="block w-full rounded-lg border border-ink-600 bg-ink-800/50 p-3 text-left transition hover:border-accent">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`inline-flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-medium ${STATUS[c.status]}`}>
                  {c.status === "closed" ? <FolderClosed size={11} /> : <FolderOpen size={11} />}{c.status}
                </span>
              </div>
              <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-500">
                <span>severity: {c.severity}</span><span>· priority {c.priority}</span>
                <span>· {c.evidence_count} evidence</span>
                {c.citizen_visible ? <span className="text-signal-green">· citizen-visible</span> : null}
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-ink-500 bg-ink-800 p-5" onClick={(e) => e.stopPropagation()}>
        <h3 className="mb-3 text-base font-semibold text-slate-100">New case</h3>
        <input placeholder="Case title" value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} className={inp + " mb-2"} />
        <textarea placeholder="Description" rows={3} value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} className={inp + " mb-2 resize-y"} />
